"""
ocr_corr2.py  —  OCR ด้วย OpenCV Template Matching
=============================================================
...
def _find_boxes(img_bgr, img_shape):
    """
    ใช้ Morphological Line Detection เหมือน TrOCR/Tesseract
    ตรวจจากเส้นกรอบกล่อง 
    """
    ...
    for cnt in contours:
        ...
        if area   < (w_img * h_img) * 0.003: continue
        if y      < h_img * 0.05:            continue
        if not (1.0 <= aspect <= 15.0):       continue
...
def extract_answers(image_path: str) -> list:
    ...
    boxes, _ = _find_boxes(img, img.shape)   # ใช้ Line Detection
...
def extract_answers_debug(image_path: str):
    ...
    bw_clean, bw_raw = _preprocess_full(img)
    boxes, box_mask = _find_boxes(img, img.shape)   # ใช้ Line Detection
    h_img, w_img = img.shape[:2]
