...
=============================================================
ใช้เทคนิค Morphological Line Detection เพื่อแยก "เส้นกล่อง" ออกจาก "ตัวหนังสือ"
แก้ปัญหากล่องไปดูดคำข้างเคียงมาติดด้วย
"""
import cv2
import numpy as np
import torch
...
def _find_boxes(bw, img_shape):
    ...
    # 1. สกัดเส้นแนวนอน (ต้องยาวอย่างน้อย 30 pixel ตัวอักษรปกติจะโดนลบทิ้งหมด)
    hor_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (30, 1))
    hor_lines = cv2.morphologyEx(bw, cv2.MORPH_OPEN, hor_kernel)
    
    # 2. สกัดเส้นแนวตั้ง (ต้องยาวอย่างน้อย 20 pixel)
...
def _recognize_trocr(roi_color):
    """ส่งภาพกล่อง (ที่เป็นสี) ไปให้ AI อ่านลายมือ"""
...
def extract_answers(image_path: str) -> list:
    ...
    for (y,x,w,h) in boxes:
        # หั่นขอบดำของเส้นกล่องทิ้งไปนิดหน่อย AI จะได้เห็นแค่ลายมือข้างใน
        px, py = max(int(w * 0.04), 2), max(int(h * 0.10), 2)
